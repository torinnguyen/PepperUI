...
{
  //Dummy image list. Copyrighted material, do not copy.
  NSArray *imageArray = [NSArray arrayWithObjects:
                         ...
                         @"s_cpj5",
                         nil];
  NSArray *fullsizeImageArray = [NSArray arrayWithObjects:
                                 @"cpj1.jpg",
                                 @"cpj2.jpg",
                                 @"cpj3.jpg",
                                 @"cpj4.jpg",
  ...
  for (int j=0; j<DEMO_NUM_PAGES; j++) {
    ...
    myPage.halfsizeURL = [imageArray objectAtIndex:j % imageCount];
    myPage.fullsizeURL = [fullsizeImageArray objectAtIndex:j % imageCount];
...
- (void)addPageToBookIndex:(int)bookIndex
{
  ...
  int existingPageCount = [theBook.pages count];
  
  Page *newPage = [[Page alloc] init];
  newPage.pageID = arc4random() % 123456;
  newPage.halfsizeURL = [imageArray objectAtIndex:(existingPageCount % imageCount)];
  newPage.fullsizeURL = [fullsizeImageArray objectAtIndex:(existingPageCount % imageCount)];
